...
def is_valid(password):
    if (
        any(ALPHABET.index(password[i-1]) == ALPHABET.index(password[i])-1 and ALPHABET.index(password[i]) == ALPHABET.index(password[i+1])-1 for i in range(1, 7))
        and not any(ch in password for ch in 'iol')
        and len(set([password[i] for i in range(1, 8) if password[i-1] == password[i]])) > 1
    ):
        return True
    return False
